   ...
   FROM gldata.patients,
    ( SELECT colnage.patient_id,
           ...
           FROM ( SELECT glcol.patient_id,
                  ...
                  WHERE patients_1.patient_id::text = glcol.patient_id::text AND patients_1.patient_id::text = n_polyps.patient_id::text AND glcol.encounter_date = n_polyps.encounter_date AND (glcol.patient_id::text IN ( SELECT colonoscopies.patient_id
                           FROM gldata.colonoscopies
                          WHERE colonoscopies.index = true AND colonoscopies.total_polyps_reported >= 2))) colnage
          WHERE colnage.age_yrs >= 0::double precision AND colnage.n_polyps_recorded >= 0
